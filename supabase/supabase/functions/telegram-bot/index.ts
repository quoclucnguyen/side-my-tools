import { Bot, webhookCallback } from "https://esm.sh/grammy@1.36.3";
...
bot.command("login", async (ctx)=>{
  ...
  try {
    ...
    const { data: existingUserByTelegramId, error: checkTelegramError } = await supabase.from('users').select('id, telegram_id, email').eq('telegram_id', telegramId).single();
    if (checkTelegramError && checkTelegramError.code !== 'PGRST116') {
    ...
    const { data: existingUserByEmail, error: checkEmailError } = await supabase.from('users').select('id, telegram_id, email').eq('email', email).single();
